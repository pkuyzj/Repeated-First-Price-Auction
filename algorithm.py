...
def Partial_Constrained(T, B, values, hobs, epsilon, delta):
    ...
    for t in range(1,T):
        ...
        # Update estimation
        NM = np.zeros(M)+(T+2023)
        if hob<=b:
            # Win in round t. The feedback is one-sided. Only estimators with k>=k_t are updated.
            ...
            # Lost in round t. The feedback is complete. All estimators are updated.
            for k in range(K):
                NK[k]+= 1.0
                if hob <= k/K:
                    C[k]+= (k/K)*1.0
        ...
        for m in range(M): 
            CB =  math.sqrt((6*(math.log(2*T,e))*(math.log(((M*K*T)/delta), e)))/(NM[m]))#confidence bound
